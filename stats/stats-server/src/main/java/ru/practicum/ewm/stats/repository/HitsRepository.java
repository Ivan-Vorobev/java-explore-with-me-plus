...
public interface HitsRepository extends JpaRepository<Hit, Long> {
    ...
    @Query("SELECT new ru.practicum.dto.HitsStatDTO(h.app, h.uri, COUNT(h) AS hits) " +
           "FROM Hit h " +
    ...
    @Query("SELECT new ru.practicum.dto.HitsStatDTO(h.app, h.uri, COUNT(DISTINCT h.ip) AS hits) " +
           "FROM Hit h " +
           "WHERE h.timestamp BETWEEN :start AND :end " +
           "GROUP BY h.app, h.uri " +
           "ORDER BY hits DESC")
    ...
    @Query("SELECT new ru.practicum.dto.HitsStatDTO(h.app, h.uri, COUNT(h) hits) " +
           "FROM Hit h " +
